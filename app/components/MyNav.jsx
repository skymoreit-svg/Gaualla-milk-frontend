...
export default function MyNav() {
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      try {
        const res = await axios.get(`${adminurl}/check`, { withCredentials: true });
        if (res.data.success) setAdminLogin(true);
      } catch {
        setAdminLogin(false);
      }
    };
  ...
  return (
    <>
      <div className="px-2 py-2 md:px-12 xl:px-32 lg:py-2 flex gap-3 justify-between items-center sticky bg-white z-[99] top-0">
        <Link href="/">
          <img src="/img/logo.webp" alt="Gaualla Milk Dairy Logo" className="w-auto h-14 lg:h-20 cursor-pointer" />
        </Link>
