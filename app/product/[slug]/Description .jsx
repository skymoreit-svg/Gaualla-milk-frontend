  ...
  return (
    <div className="w-full max-w-screen-2xl mx-auto px-4 md:px-12 xl:px-32 py-12">
      ...
      <div className="flex items-center justify-center mb-12">
        <div className="inline-flex p-1 bg-background00 rounded-2xl overflow-x-auto scrollbar-hide max-w-full">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold transition-all duration-300 whitespace-nowrap ${
                activeTab === tab.id
                  ? "bg-background text-[var(--primary)] shadow-md scale-105"
          ...
          >
                ...
                <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="p-6 bg-[var(--primary)]/5 rounded-2xl border border-[var(--primary)]/10">
                    <h4 className="font-bold text-[var(--primary)] mb-3 flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full bg-[var(--primary)]" /> Why Choose Gaualla?
                    </h4>
                    <p className="text-sm text-text">Our products are 100% natural, ethically sourced, and delivered fresh to maintain peak nutritional value.</p>
                  </div>
                  <div className="p-6 bg-[var(--primary)]/5 rounded-2xl border border-[var(--primary)]/10">
                    <h4 className="font-bold text-[var(--primary)] mb-3 flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full bg-[var(--primary)]" /> Sustainable Quality
                    </h4>
                    <p className="text-sm text-text">We prioritize eco-friendly packaging and traditional processes that preserve the soul of our dairy.</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === "additional-info" && (
              <div className="max-w-4xl mx-auto space-y-8">
                <div className="bg-background p-8 rounded-3xl border border-highlight shadow-sm">
                  <h3 className="text-2xl font-bold text-text mb-4">Shipping & Delivery Policies</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    ...
                    <div key={i} className="p-6 border border-highlight rounded-2xl hover:border-[var(--primary)] transition-colors">
                      <h5 className="font-bold text-text mb-1">{item.title}</h5>
                      <span className="text-[var(--primary)] text-xs font-black uppercase tracking-wider">{item.time}</span>
            ...
            {activeTab === "reviews" && (
              <div className="max-w-5xl mx-auto">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-16">
                  {/* Review Summary */}
                  <div className="lg:col-span-1 bg-background p-8 rounded-3xl text-center">
                    <h4 className="text-sm font-black uppercase tracking-widest text-gray-[#252729b8] mb-4">Total Rating</h4>
                    <div className="text-6xl font-black text-text mb-4">4.8</div>
                    <div className="flex justify-center text-highlight gap-1 mb-4">
                      {[...Array(5)].map((_, i) => <Star key={i} size={20} fill="currentColor" />)}
                    </div>
                    <p className="text-gray-700 text-sm">Based on 1,248 reviews</p>
                     <button className="mt-8 w-full py-4 bg-text text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-[var(--primary)] transition-all">
                  ...
                  <div className="lg:col-span-2 space-y-8">
                    {[
                      ...
                      <div key={i} className="pb-8 border-b border-highlight last:border-0">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className="text-xs text-gray-[#252729b8] font-medium">{rev.date}</span>
                        </div>
                        <h5 className="font-bold text-text mb-2">{rev.title}</h5>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-text">{rev.name}</span>
                        </div>
                      </div>
                    ))}
                    <button className="flex items-center gap-2 text-[var(--primary)] font-bold hover:gap-4 transition-all uppercase tracking-widest text-xs mt-4">
